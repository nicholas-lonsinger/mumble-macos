...
bool SelfSignedCertificate::generateMumbleCertificate(QString name, QString email, QSslCertificate &qscCert,
	...
	QString commonName         = name.isEmpty() ? QLatin1String("Mumble User") : name;
	...
	CHECK(subjectName = X509_get_subject_name(x509));

	CHECK(X509_NAME_add_entry_by_txt(subjectName, "CN", MBSTRING_UTF8,
									 reinterpret_cast< unsigned char * >(commonName.toUtf8().data()), -1, -1, 0));

	CHECK(X509_set_issuer_name(x509, subjectName));
	...
	if (!email.trimmed().isEmpty()) {
		CHECK(add_ext(x509, NID_subject_alt_name, QString::fromLatin1("email:%1").arg(email).toUtf8().data()));
	}
