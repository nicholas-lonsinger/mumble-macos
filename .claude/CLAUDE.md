...
- **`PKCS12Encoder` uses `pbeWithSHA1And3-KeyTripleDES-CBC` for the key bag and HMAC-SHA1 for the outer MAC** — the legacy-but-universal flavor, not because we like DES but because openssl, `SecPKCS12Import`, and every Mumble-era tool reliably interoperate on it. PBES2+AES is picky across parsers and gains us nothing for a format only Mumble cares about.
- **`PKCS12Encoder.encode` self-checks via `SecPKCS12Import` before returning. Don't remove this.** ~600 lines of hand-rolled DER/ASN.1 is exactly the kind of code where an encoder bug would otherwise surface as an opaque TLS-handshake failure much later. The RFC 7292 Appendix B KDF in `PKCS12KDF` is likewise hand-rolled — it's a PKCS#12-specific algorithm not exposed by CommonCrypto or CryptoKit, so don't go hunting for an Apple API that replaces it.
- **Self-created identities get a random 32-byte internal P12 password; the user never sees it.** Export does *not* dump the envelope's raw bytes — it re-wraps through `PKCS12Encoder` under the user's chosen password, so what they remember is what they typed.
